...
ifeq (, $(shell which certbot))
	...
	@certbot certonly --standalone -d $(DOMAIN) -d api.$(DOMAIN) -d admin.$(DOMAIN)  -d www.$(DOMAIN) -d profile.$(DOMAIN)
...
test: 
	...
	@nginx -t
	@echo "$(C_BLUE)<-- nginx test end -->$(C_RESET)"

### exploring parallel solutions
### https://stackoverflow.com/questions/23814510/multi-threaded-make
clean:
	@rm -rf $(DIR_API)/node_modules
	@rm -rf $(DIR_APP)/node_modules
	@rm -rf $(DIR_ADMIN)/node_modules
	@rm -rf $(DIR_PROFILE)/node_modules
	@echo	"node_modules clean complete"

build:
	@cd $(DIR_API) && npm i --only=production && npm run build
	@cd ../$(DIR_APP) && npm i --only=production && npm run build
	@cd ../$(DIR_ADMIN) && npm i --only=production && npm run build
	@cd ../$(DIR_PROFILE) && npm i --only=production && npm run build
...
start-api:
	@echo "start $(DIR_API)"
	@cd $(DIR_API) && pm2 start pm2-prod.json
	@cd ..
start-app:
	@echo "start $(DIR_APP)"
	@cd $(DIR_APP) && pm2 start pm2-prod.json
	@cd ..
end:
	@pm2 stop api app
